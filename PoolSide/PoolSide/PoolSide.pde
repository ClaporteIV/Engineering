...
void draw() {
  ...
  stroke(255);
  parseControlValues();
  //print("after parse nums ");
  //println(thrustValues);
  ...
  for (int i = 0; i < NUM_MOTORS; i++) {
    ...
    if (thrustValues[i] != previousValues[i]) {
      ...
      for (int j = 0; j < NUM_MOTORS; j++) {
        ...
        //sendString = "http://169.254.175.59:5000/motordata?motor0=-10&motor1=-1&motor2=1&motor3=1&motor4=1&";  //For debugging
        //sendString = "http://169.254.175.59:5000/motorData?motor0=237&motor1=-90&motor2=237&motor3=-90&motor4=0&";
        //println(sendString);
...
float[] getTranslation(float x, float y) {
  ...
  for (int i = 0; i < NUM_HORIZ_MOTORS; i++) {
    vals[i] = map(vals[i], -360.624, 360.624, -255, 255);
